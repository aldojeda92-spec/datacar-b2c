...
export default function WizardContainer() {
  ...
  // --- MOTOR FINANCIERO INVERSO ---
  const calculateFinanceTarget = () => {
    const r = 0.09 / 12; // 9% Anual
    const n = financeParams.term;
    const q = financeParams.installment;
    const p_loan = q * ((1 - Math.pow(1 + r, -n)) / r);
    // Factor de retención 2.7% (1 - 0.027 = 0.973)
    const p_total = (p_loan / 0.973) + financeParams.delivery;
    return { min: p_total * 0.9, max: p_total * 1.1 };
  };
  ...
  const handleExecute = async () => {
    ...
    if (paymentMode === 'financed') {
      const range = calculateFinanceTarget();
            ...
            <label key={item} className="flex items-start gap-3 p-2.5 hover:bg-slate-50 cursor-pointer rounded transition-colors">
              ...
              <div className="flex flex-col">
                ...
                {descriptions && descriptions[item] && (
                  <span className="text-[9px] font-medium text-slate-400 mt-0.5">{descriptions[item]}</span>
                )}
              </div>
            </label>
          ))}
  ...
  if (showComparison) {
      // (Mantenido lógica existente)
      return <div className="p-10 text-center"><h1 className="text-2xl font-bold">Vista de Matriz</h1><button onClick={() => setShowComparison(false)} className="mt-4 bg-slate-900 text-white px-4 py-2 rounded">Cerrar</button></div>
  }
      ...
      {step === 1 && (
        <div className="max-w-4xl mx-auto p-6 md:p-10">
          <div className="bg-white border border-slate-100 p-8 md:p-12 shadow-xl rounded-xl space-y-10">
            ...
            {/* === INYECCIÓN: MODALIDAD DE BÚSQUEDA === */}
      ...
      {step === 2 && (
        <div className="max-w-[1700px] mx-auto p-4 md:p-8 pb-40 animate-in fade-in duration-700 space-y-10">
          <div className="bg-[#006837] p-8 md:p-10 text-white rounded-lg shadow-2xl border-l-8 border-[#FFD100]">
            <h2 className="font-extrabold text-xl uppercase tracking-tight">Análisis de Mercado Homologado</h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
            {displayedAutos.map((auto) => {
              ...
              // INYECCIÓN: Si está en financiado, calculamos la cuota para mostrarla
              ...
              return (
                <div key={auto.id} className="bg-white border border-slate-100 rounded-lg shadow-sm p-5 space-y-4">
                  ...
                  {/* INYECCIÓN: Toggle de precio vs cuota */}
                  ...
                  <a href="#" className="block w-full py-2 bg-[#006837] text-white text-center font-bold text-[10px] uppercase tracking-widest rounded-sm">Consultar</a>
                </div>
              );
            })}
          </div>
