...
export default function WizardContainer() {
  ...
  const handleExecuteAnalysis = async () => {
    ...
    if (result.success) {
      ...
      } catch (e) { alert("Fallo de red"); } finally { setIsAnalyzing(false); }
    }
  };
  ...
  if (isAnalyzing) return (
    <div className="max-w-[1600px] mx-auto p-12 text-center py-48 bg-white">
      ...
      <p className="mt-4 text-slate-400 font-bold uppercase text-[10px] tracking-widest animate-pulse">Analizando 436 activos con Gemini 2.5 Pro...</p>
    </div>
  );
  ...
  return (
    <div className={`max-w-[1600px] mx-auto p-8 font-inter ${step === 2 ? 'bg-[#F9FAFB]' : 'bg-white'}`}>
      ...
      {step === 1 && (
        <div className="max-w-4xl mx-auto space-y-10 bg-white p-10 border border-slate-100 shadow-sm">
          
          <section>
            <h2 className="font-montserrat font-black text-[12px] uppercase tracking-widest mb-6 border-l-4 border-[#00BFFF] pl-3 text-[#0A1F33]">1. Identificación</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input name="nombre" value={formData.nombre} onChange={handleInputChange} placeholder="Nombre Completo *" className="p-3.5 border border-slate-200 outline-none text-sm bg-slate-50/30" />
              <input name="celular" value={formData.celular} onChange={handleInputChange} placeholder="Celular *" className="p-3.5 border border-slate-200 outline-none text-sm bg-slate-50/30" />
            </div>
          </section>

          <section>
            <div className="flex justify-between items-end mb-6">
              <h2 className="font-montserrat font-black text-[12px] uppercase tracking-widest border-l-4 border-[#00BFFF] pl-3 text-[#0A1F33]">2. Presupuesto (USD)</h2>
            ...
            <input type="range" name="presupuestoMin" min="5000" max="150000" step="1000" value={formData.presupuestoMin} onChange={handleRangeChange} className="w-full h-1 bg-slate-100 mb-6 accent-[#0A1F33]" />
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-2.5">
              {['SUV', 'Pickup', 'Sedán', 'Hatchback', 'Furgón'].map(t => (
                <button key={t} onClick={() => toggleTipo(t)} className={`px-5 py-2.5 text-[11px] font-black uppercase tracking-widest border transition-all ${formData.tipos.includes(t) ? 'bg-[#0A1F33] text-white' : 'bg-transparent text-[#3A3A3C] border-slate-200 hover:bg-slate-50'}`}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <section>
            <h2 className="font-montserrat font-black text-[12px] uppercase tracking-widest mb-6 border-l-4 border-[#00BFFF] pl-3 text-[#0A1F33]">4. Filtros Estratégicos</h2>
          ...
          <section>
            <h2 className="font-montserrat font-black text-[12px] uppercase tracking-widest mb-6 border-l-4 border-[#00BFFF] pl-3 text-[#0A1F33]">5. Prioridades IA</h2>
            <div className="flex flex-wrap gap-2.5">
              {['Seguridad', 'Dimensiones', 'Rendimiento', 'Precio', 'Tecnología'].map(at => (
                <button key={at} onClick={() => toggleAtributo(at)} className={`px-5 py-2.5 text-[11px] font-black uppercase tracking-widest border ${formData.atributos.includes(at) ? 'bg-[#0A1F33] text-white border-[#0A1F33]' : 'bg-transparent text-slate-300 border-slate-200'}`}>
                  {at}
                </button>
              ))}
            </div>
          ...
          <section>
            <h2 className="font-montserrat font-black text-[12px] uppercase tracking-widest mb-5 text-[#0A1F33]">6. Notas Adicionales</h2>
            <textarea name="notasAdicionales" value={formData.notasAdicionales} onChange={handleInputChange} placeholder="Escribe aquí detalles específicos..." className="w-full p-4 border border-slate-200 outline-none text-sm min-h-[120px] bg-slate-50/30" />
          </section>

          <div className="text-right pt-8 border-t border-slate-100">
            <button disabled={!formData.nombre || !formData.celular} onClick={handleExecuteAnalysis} className="bg-[#0A1F33] text-white px-16 py-5 font-montserrat font-black text-xs uppercase tracking-[3px] hover:bg-[#00BFFF] hover:text-[#0A1F33] transition-all disabled:opacity-10 w-full md:w-auto">Generar Dossier →</button>
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="animate-in fade-in zoom-in-95 duration-1000">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {top10.map((auto) => (
              <div key={auto.id} className="bg-white p-6 border border-slate-100 shadow-sm flex flex-col gap-5 hover:border-[#00BFFF] transition-all group">
                <div className="bg-slate-100 h-32 w-full flex items-center justify-center relative overflow-hidden">
                ...
                <div className="space-y-1.5">
                  <h4 className="font-montserrat font-black text-[14px] text-[#0A1F33] uppercase leading-tight group-hover:text-[#00BFFF]">{auto.marca} <span className="font-medium text-slate-500">{auto.modelo}</span></h4>
                  <p className="text-[9px] text-slate-400 uppercase font-black bg-slate-50 border px-2 py-1 inline-block">{auto.version} | {auto.origen}</p>
                </div>
                <div className="mt-auto border-t pt-4 space-y-3">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <span className="bg-[#0A1F33]/5 text-[#0A1F33] text-[9px] font-black px-2 py-1 border text-center uppercase tracking-tighter">{auto.etiqueta_principal}</span>
                  </div>
                </div>
              </div>
            ))}
